...
const ProductList: React.FC = () => {
  ...
  // Vertical Products (infinite scroll)
  const {
    data: verticalPages,
  ...
  const loadMoreHorizontal = async () => {
    try {
      ...
      console.error('আরো প্রোডাক্ট লোড করতে সমস্যা হয়েছে!', error);
    }
  };

  if (horizontalLoading || verticalInitialLoading) {
        ...
        >
          {horizontalProducts.map((product) => (
            <div
              ...
              className="relative w-56 h-72 border rounded-lg flex-shrink-0 cursor-pointer"
            >
              <img
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {verticalProducts.map((product) => (
            <div
              key={product._id}
              onClick={() => navigate(`/details/${product._id}`)}
              className="relative w-full h-72 border rounded-lg cursor-pointer"
            >
              <img
                src={product.images[0]}
                alt={product.name}
